...
SNsurvyBayes <- function (dat, transfm = c('none', 'none'), mod, smpl.n = 50,
                          num.knots = 15, n.iter = 1e3, n.chains = 3,
                          seed = c(1, 1),
                          percentiles = c(0.01, 0.05, 0.1, 0.25, 0.5,
                                          0.75, 0.9, 0.95, 0.99),
                          doSample = TRUE, doFigure = FALSE) {
  ...
  if (doSample) {
    ...
    if (mod == 'SN-B2PSP') {
      moddat.stan <- list(n1 = nrow(moddat$smpl),
                          n2 = n.nonsmpl,
                          p = ncol(X),
                          K = num.knots,
                          y = moddat$smpl$y,
                          ...
                          predX = X.pred,
                          predZ = Z.pred)
      fit <- stan(file = "SN-B2PSP-prediction.stan", data = moddat.stan,
                  iter = 100, chains = n.chains, seed = seed[2])
    ...
    if (mod == 'SN-B2PSP-cauchy') {
      moddat.stan <- list(n1 = nrow(moddat$smpl),
                          ...
                          predX = X.pred,
                          predZ = Z.pred)
      fit <- stan(file = "SN-B2PSP-cauchy.stan", data = moddat.stan, iter = 100,
                  chains = n.chains, seed = seed[2])
      fit <- stan(fit = fit, data = moddat.stan, iter = n.iter, chains = n.chains)
      print(fit, c('beta', 'b', 'lambda', 'nu', 'alpha', 'lp__') )
    }
	...
	if (mod == 'SN-B2PSP-uniform') {
      moddat.stan <- list(n1 = nrow(moddat$smpl),
                          ...
                          predX = X.pred,
                          predZ = Z.pred)
      fit <- stan(file = "SN-B2PSP-uniform.stan", data = moddat.stan,
                  iter = 100, chains = n.chains, seed = seed[2])
      fit <- stan(fit = fit, data = moddat.stan, iter = n.iter, chains = n.chains)
      print(fit, c('beta', 'b', 'lambda', 'nu', 'alpha', 'lp__') )
    }
    ...
    if (mod == 'SN-B2PSP-omega') {
      moddat.stan <- list(n1 = nrow(moddat$smpl),
                          ...
                          predX = X.pred,
                          predZ = Z.pred)
      fit <- stan(file = "SN-B2PSP-omega.stan", data = moddat.stan,
                  iter = 100, chains = n.chains, seed = seed[2])
      fit <- stan(fit = fit, data = moddat.stan, iter = n.iter, chains = n.chains)
      print(fit, c('beta', 'b', 'lambda', 'nu', 'alpha', 'lp__') )
    }
    ...
    if (mod == 'SN-poly') {
      moddat.stan <- list(n1 = nrow(moddat$smpl),
                          ...
                          predX = X.pred,
                          predZ = Z.pred,
                          Pi = origdat$smpl$probs,
                          predPi = origdat$nonsmpl$probs)
      fit <- stan(file = "SN-poly-prediction.stan", data = moddat.stan,
                  iter = 50, chains = n.chains, seed = seed[2])
      fit <- stan(fit = fit, data = moddat.stan, iter = n.iter, chains = n.chains)
    ...
    if (mod == 'SN-poly-cauchy') {
      moddat.stan <- list(n1 = nrow(moddat$smpl),
                          ...
                          Pi = origdat$smpl$probs,
                          predPi = origdat$nonsmpl$probs)
      fit <- stan(file = "SN-poly-cauchy.stan", data = moddat.stan, iter = 50,
                  chains = n.chains, seed = seed[2])
      fit <- stan(fit = fit, data = moddat.stan, iter = n.iter, chains = n.chains)
	...
	if (mod == 'SN-poly-uniform') {
      moddat.stan <- list(n1 = nrow(moddat$smpl),
                          ...
                          Pi = origdat$smpl$probs,
                          predPi = origdat$nonsmpl$probs)
      fit <- stan(file = "SN-poly-uniform.stan", data = moddat.stan,
                  iter = 50, chains = n.chains, seed = seed[2])
      fit <- stan(fit = fit, data = moddat.stan, iter = n.iter, chains = n.chains)
      print(fit, c('beta', 'b', 'sigma', 'gamma', 'alpha', 'lp__') )
    }
	...
	if (mod == 'SN-poly-omega') {
      moddat.stan <- list(n1 = nrow(moddat$smpl),
                          ...
                          y = moddat$smpl$y,
                          X = X, Z = Z,
                          predX = X.pred, predZ = Z.pred,
                          Pi = origdat$smpl$probs, predPi = origdat$nonsmpl$probs)
      fit <- stan(file = "SN-poly-omega.stan", data = moddat.stan, iter = 50,
                  chains = n.chains, seed = seed[2])
      fit <- stan(fit = fit, data = moddat.stan, iter = n.iter, chains = n.chains)
      print(fit, c('beta', 'b', 'sigma', 'gamma', 'alpha', 'lp__') )
    }
    ...
    yrep <- sapply(1:chain.length, function (t) {
      yrep.iter <- rsn(n = smpl.n, xi = xi.draws[t, ],
                       omega = omega.draws[t, ], alpha = alpha.draws[t])
      return( yrep.iter )
    } )
